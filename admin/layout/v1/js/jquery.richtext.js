(function ( $ ) {
    ...
    $.fn.richText = function( options ) {
        ...
        var $dropdownOuter = $('<br>', {class: "richText-dropdown-outer"});
        ...
        var $dropdownList = $('<ul />', {class: "richText-dropdown"}), // dropdown lists
            $dropdownBox = $('<br >', {class: "richText-dropdown"}), // dropdown boxes / custom dropdowns
            $form = $('<br >', {class: "richText-form"}), // symbolic form
            $formItem = $('<br >', {class: 'richText-form-item'}), // form item
        ...
        $editor.find(".richText-help").on("click", function() {
            ...
            if($editor) {
                var $outer = $('<br>', {class: 'richText-help-popup', style: 'position:absolute;top:0;right:0;bottom:0;left:0;background-color: rgba(0,0,0,0.3);'});
                var $inner = $('<br>', {style: 'position:relative;margin:60px auto;padding:20px;background-color:#FAFAFA;width:70%;font-family:Calibri,Verdana,Helvetica,sans-serif;font-size:small;'});
                var $content = $('<br>', {html: '<span id="closeHelp" style="display:block;position:absolute;top:0;right:0;padding:10px;cursor:pointer;" title="' + settings.translations.close + '"><span class="fa fa-times"></span></span>'});
        ...
        $(document).on('contextmenu', '.richText-editor', function(e) {
            ...
            if($target.prop("tagName") === "A") {
                ...
                $list.find('.fa-image').on('click', function() {
                    var align;
                    if($target.parent('br').length > 0 && $target.parent('br').attr('style') === 'text-align:center;') {
                        align = 'center';
                    } else {
        ...
        $(document).on("click", "#richText-Video button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<br>', {style: 'color:red;display:none;', class: 'form-item is-error', text: settings.translations.pleaseEnterURL}));
                    ...
                    if(!html) {
                        $form.prepend($('<br>', {style: 'color:red;display:none;', class: 'form-item is-error', text: settings.translations.videoURLnotSupported}));
                        $form.children('.form-item.is-error').slideDown();
                        setTimeout(function() {
                            $form.children('.form-item.is-error').slideUp(function () {
                                $(this).remove();
        ...
        $(document).on("click", "#richText-Image button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<br>', {style: 'color:red;display:none;', class: 'form-item is-error', text: settings.translations.pleaseSelectImage}));
                    $form.children('.form-item.is-error').slideDown();
                    setTimeout(function() {
                        $form.children('.form-item.is-error').slideUp(function () {
                            $(this).remove();
                    ...
                    if($editNode.length > 0 && $editNode.prop("tagName") === "IMG") {
                        ...
                        if($editNode.parent('br').length > 0 && $editNode.parent('br').attr('style') === 'text-align:center;' && align !== 'center') {
                            $editNode.unwrap('<br>');
                            $editNode.attr('align', align);
                        } else if(($editNode.parent('br').length === 0 || $editNode.parent('br').attr('style') !== 'text-align:center;') && align === 'center' ) {
                            $editNode.wrap('<br>');
        ...
        $(document).on("click", "#richText-File button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<br>', {style: 'color:red;display:none;', class: 'form-item is-error', text: settings.translations.pleaseSelectFile}));
                    $form.children('.form-item.is-error').slideDown();
                    setTimeout(function() {
                        $form.children('.form-item.is-error').slideUp(function () {
                            $(this).remove();
            ...
            if($editor.length > 0 && id === editorID && (!$button.parent("li").attr('data-disable') || $button.parent("li").attr('data-disable') === "false")) {
                ...
                if(command === "toggleCode") {
                    ...
                    if (command === "removeFormat") {
                        ...
                        formatText('formatBlock', 'br', id);
                    }
                    // clean up empty tags, which can be created while replacing formatting or when copy-pasting from other tools
                    $editor.find('br:empty,p:empty,li:empty,h1:empty,h2:empty,h3:empty,h4:empty,h5:empty,h6:empty').remove();
        ...
        function fixFirstLine() {
            if($editor && !$editor.find(".richText-editor").html()) {
                ...
                if(settings.useParagraph !== false) {
                    ...
                    $editor.find(".richText-editor").html('<br>');
                }
            } else {
                // replace tags, to force <div> or <p> tags and fix issues
                if(settings.useParagraph !== false) {
                    $editor.find(".richText-editor").find('br:not(.videoEmbed)').replaceWith(function() {
                        return $('<p />', {html: $(this).html()});
                    });
                } else {
                    $editor.find(".richText-editor").find('p').replaceWith(function() {
                        return $('<br>', {html: $(this).html()});
                    });
                }
            }
            updateTextarea();
